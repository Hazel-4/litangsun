...
class ImageDataGenerator(object):
    ...
    def __init__(self, dataset, mode, batch_size, num_classes=None, shuffle=False, buffer_size=1000):
        ...
        # tang新加的代码{
            ...
            self.labels = convert_to_tensor(self.labels, dtype=tf.int32)
            # 利用tensorflow的dataset接口创建数据集
            data = tf.data.Dataset.from_tensor_slices((self.img_paths, self.img_ids, self.labels))
            data = data.map(self._parse_function_extract_features, num_parallel_calls=8).prefetch(100 * batch_size)
        # tang新加的代码}    
    ...
    # tang新加的代码
    def _parse_function_extract_features(self, filename, img_ids, label):
        # 将标签转为one-hot编码
        # one_hot = tf.one_hot(label, self.num_classes)
        # 加载图片的预处理
        img_string = tf.read_file(filename)
        img_decode = tf.image.decode_jpeg(img_string, channels=3)
        img_resized = tf.image.resize_images(img_decode, [224, 224])
        return img_resized, img_ids, label
