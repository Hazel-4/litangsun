...
run.FN = 0;
for j = 1:num_data,
    if (run.bag_pred(j,3) == 1 && run.bag_pred(j,4) ==1)
         run.TP = run.TP+1;
    elseif (run.bag_pred(j,3) == 1 && run.bag_pred(j,4) == 0)
        run.FP = run.FP + 1;
    elseif (run.bag_pred(j,3) == 0 && run.bag_pred(j,4) == 0)
        run.TN = run.TN + 1;
    elseif (run.bag_pred(j,3) == 0 && run.bag_pred(j,4) == 1)
        run.FN = run.FN + 1;
    end
end
run.TPR = run.TP/(run.TP + run.FN);
run.TNR = run.TN/(run.TN + run.FP)
