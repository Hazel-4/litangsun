...
OutputResult = [];
if (isfield(run, 'BagAccu')), OutputResult = [OutputResult sprintf('ACC = %f, ', run.BagAccu*100)]; end;
if (isfield(run, 'TPR')), OutputResult = [OutputResult sprintf('TPR = %f, ', run.TPR)]; end;
if (isfield(run, 'TNR')), OutputResult = [OutputResult sprintf('TNR = %f, ', run.TNR)]; end;
...
if (~isempty(pred_file))
    ...
    fprintf(fid, 'Index\tProb\tPredict\tTruth\n');
    fprintf(fid, '%d\t%d\t%d\t%d\n', run.bag_pred');
