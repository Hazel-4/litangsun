...
%tang加的代码
%获取测试数据的示例条数
[num_test_inst,~] = size(test_instance);
if isempty(train_bags)
    ...
    [test_label_predict, test_prob_predict] = LibLinear(para, [], [], test_instance, ones(num_test_inst, 1));    
else
    ...
    for i = 1: num_train_bag
        ...
        if train_bags(i).label == 0
            ...
            sample_instance(idx+1, :) = mean(train_bags(i).instance, 1); %返回包中特征矩阵每一列的均值
    ...
    while 1
        ...
        [all_label_predict, all_prob_predict] = LibLinear([para new_para], sample_instance, sample_label, [train_instance; test_instance], ones(num_train_inst+num_test_inst, 1));
