...
class ResNetModel(object):
    ...
    def __init__(self, is_training, depth=50, num_classes=2):
    ...
    def inference(self, x):
        ...
        with tf.variable_scope('scale5'):
            self.s5 = stack(s4, is_training=self.is_training, num_blocks=self.num_blocks[3], stack_stride=2, block_filters_internal=512)

        # post-net
        self.avg_pool = tf.reduce_mean(self.s5, reduction_indices=[1, 2], name='avg_pool')

        with tf.variable_scope('fc'):
            self.prob = fc(self.avg_pool, num_units_out=self.num_classes)
