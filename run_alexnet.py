...
Comparision_txt = "data/2016_txt/Comparision.txt"
test_txt = "data/2016_txt/fold5.txt"
...
filewrite_path = "tensorboard/alexnet"
checkpoint_path = "checkpoints/alexnet/fold1"
...
def train():
    ...
    print('每轮迭代次数：%d' % train_batches_per_epoch)
...
def genrate_pre_result():
    ...
    # 设置模型文件的路径
    model_path = "checkpoints/alexnet/model_epoch3_0.5039.ckpt"
    with tf.Session() as sess:
...
def evaluation_eval_dataset():
    # 读取验证集的csv文件
    data = pd.read_csv("/media/Store/tyh/funi_across_validation/test_1.csv", index_col=False)
    with tf.device("/cpu:0"):
    ...
    # 设置模型文件的路径
    model_path = "checkpoints/alexnet/model_epoch22_0.9121.ckpt"
    print(model_path)
    with tf.Session() as sess:
        sess.run(val_init_op)
        saver.restore(sess, model_path)
...
def feature_layer7():
    ...
    # 设置模型文件的路径
    model_path = "checkpoints/alexnet/model_epoch22_0.9570.ckpt"
    with tf.Session() as sess:
        sess.run(test_init_op)
        # 加载模型文件
        saver.restore(sess, model_path)
        for step in range(steps):
            ...
            # 预测图片的标签
            feature = sess.run(output_layer7, feed_dict={x: image_data, keep_prob: 1.0})
        ...
        # exit()
        save_txt_path = os.path.join(save_txt_dir, "alexnet_feature.txt")
